...
#if (USE_CUSTOM_RANGING_VL53L4ED == 1U)
...
int32_t CUSTOM_RANGING_SENSOR_Init(uint32_t Instance)
{
  ...
  else
  {
    ...
    {
#if (USE_CUSTOM_RANGING_VL53L4ED == 1U)
      case CUSTOM_VL53L4ED:
        if (VL53L4ED_Probe(Instance) != BSP_ERROR_NONE)
        {
...
static int32_t VL53L4ED_Probe(uint32_t Instance)
{
  ...
  VL53L4ED_IO_t              IOCtx;
  uint32_t                   id;
  static VL53L4ED_Object_t   VL53L4EDObj;

  /* Configure the ranging sensor driver */
  IOCtx.Address     = RANGING_SENSOR_VL53L4ED_ADDRESS;
  IOCtx.Init        = CUSTOM_VL53L4ED_I2C_INIT;
  IOCtx.DeInit      = CUSTOM_VL53L4ED_I2C_DEINIT;
  IOCtx.WriteReg    = CUSTOM_VL53L4ED_I2C_WRITEREG;
  IOCtx.ReadReg     = CUSTOM_VL53L4ED_I2C_READREG;
  IOCtx.GetTick     = BSP_GetTick;

  if (VL53L4ED_RegisterBusIO(&VL53L4EDObj, &IOCtx) != VL53L4ED_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else if (VL53L4ED_ReadID(&VL53L4EDObj, &id) != VL53L4ED_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
  {
    ...
    {
      CUSTOM_RANGING_Drv[Instance] = (RANGING_SENSOR_Drv_t *) &VL53L4ED_RANGING_SENSOR_Driver;
      CUSTOM_RANGING_CompObj[Instance] = &VL53L4EDObj;

      if (CUSTOM_RANGING_Drv[Instance]->Init(CUSTOM_RANGING_CompObj[Instance]) != VL53L4ED_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else if (CUSTOM_RANGING_Drv[Instance]->GetCapabilities(CUSTOM_RANGING_CompObj[Instance],
                                                             &RANGING_SENSOR_Cap[Instance]) != VL53L4ED_OK)
      {
        ret = BSP_ERROR_COMPONENT_FAILURE;
      }
      else
...
{
#if (USE_CUSTOM_RANGING_VL53L4ED == 1U)
  HAL_GPIO_WritePin(CUSTOM_VL53L4ED_XSHUT_PORT, CUSTOM_VL53L4ED_XSHUT_PIN, GPIO_PIN_RESET);
  HAL_Delay(2);
  HAL_GPIO_WritePin(CUSTOM_VL53L4ED_XSHUT_PORT, CUSTOM_VL53L4ED_XSHUT_PIN, GPIO_PIN_SET);
  HAL_Delay(2);
#endif /* Use custom ranging */
}
